...
func (r *pilotSource) GetConfigurations() (map[string]interface{}, error) {
	fmt.Println("Raj: GetConfigurations")
	egressConfigs, err := r.getEgressConfigFromPilot()
	fmt.Println("Raj after getting ocnfiguration form getconfiguation %v", egressConfigs)
	if err != nil {
...
func (r *pilotSource) getEgressConfigFromPilot() ([]*egressmodel.EgressRule, error) {
    fmt.Println("Raj: getEgressConfigFromPilot")
	clusters, _  :=  r.fetcher.GetAllClusterConfigurations()
	//fmt.Println("Raj: After getting configuration form getallclusterconfiguration", clusters)
	var egressRules []*egressmodel.EgressRule
	for _, cluster := range clusters {
		fmt.Println("Raj: ClusterName  %v", cluster.Name)
		fmt.Println("Raj: Cluster type %v", cluster.Type)
		var rule egressmodel.EgressRule

		if cluster.Type == 4{
            fmt.Println("Raj: Entering inside becuase the type is orginal_dst")
			data := strings.Split(cluster.Name, "|")
			fmt.Println("Raj value of data %v", data)
			if len(data) > 1 && data[0] == OUTBOUND{
				intport, err := strconv.Atoi(data[1])
				fmt.Println("Raj: valur if int port", intport)
				if err != nil {
					return nil, nil
				}
	...
	//egressRules := clusterToEgressRule(clusters)
	fmt.Println("Raj: filtered value of egress rules %v", egressRules)
...
func (r *pilotEventListener) Event(e *core.Event) {
	...
	if v == nil {
		...
		lager.Logger.Infof("[%s] egress rule of piot is removed", e.Key)
		return
	}
